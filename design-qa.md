...
- Header portrait: the 44 px avatar reuses that same source file, with a tighter local crop so the portrait remains recognizable at navigation size.
...
- Header iteration: replaced the separate avatar asset with the About portrait and kept the source unoptimized; only the small square presentation is cropped and magnified.
